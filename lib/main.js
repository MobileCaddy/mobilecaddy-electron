...
function getLastRow(table, success) {
  smartstore.get("SELECT * from " + table + " ORDER BY _soupEntryId DESC LIMIT 1", [], function(err, row){
    console.log("getLastRow row", row);
    success([row]);
  })
}
  ...
  if (args.entries.length > 0) {
    ...
    if (insertEntries.length > 0) {
      for (var i = 0; i < insertEntries.length;  i++) {
        ...
        sql = "INSERT OR IGNORE INTO " + args.table + " (" + cols + ") VALUES (" + vals + ")";
        console.log("sql", sql);
        smartstore.get(sql, [], function(err, row){
          if (!err) {
            if (insertEntries.length ==1) {
              // we are inserting 1 rec, so may want the soupEntryId
              getLastRow(args.table, success);
              returned = true;
            } else {
          ...
          // if (i == insertEntries.length && insertEntries.length != 1 && updateEntries.length === 0) {
          //   success(resultRows);
          // }
        });
      }
    ...
    if (updateEntries.length > 0 && !returned) {
      let entryIdStr = "";
      smartstore.serialize(function() {
        updateEntries.forEach(function(entry) {
          ...
          for (var propertyName in entry) {
            let val = (typeof(entry[propertyName]) === "string" ) ? "'" + entry[propertyName] + "'" : entry[propertyName];
            if (val !== null) vals += propertyName + "=" + val + ",";
          }
          vals = vals.slice(0, vals.length - 1);
          let sql = "UPDATE " + args.table + " SET "  + vals + " WHERE " + args.externalIdPath + " = '" + entry[args.externalIdPath] +"'";
          console.log("sql", sql);
          smartstore.run(sql);
        });
        ...
        smartstore.all(sql, [], function(err, rows){
          if (!err) {
            ...
            success(resultRows.concat(rows));
          } else {
            console.log("err", err);
            myError = err;
          }
...
function smartstoreUpsertSoupEntriesOrig(args, success) {
  console.log("smartstoreUpsertSoupEntries", args);
  // var stmt = smartstore.prepare("INSERT INTO " + args.table + " VALUES (?)");
  // cols = "Name, CurrentValue";
  // vals = ['Test', 'Test2', 'Test3'];
  // stmt.run(vals, function(stmt2){
  //   console.log("stmt2", stmt2);
  //   });
  // stmt.finalize();

  // smartstore.run("INSERT INTO " + args.table + " VALUES ('Test1', 'Test2', 'Test3')", function(err, row){
  //   console.log("err", err);
  //   console.log("row", row);
  // });
  if (args.entries.length > 0) {
    for (var i = 0; i < args.entries.length;  i++) {
      ...
      let rows = [];
      let myError = null;
      if (args.entries[i]._soupEntryId) {
        let entry = args.entries[i];
        // UPDATE - IS THIS RIGHT?
        for (var propertyName in args.entries[i]) {
          let val = (typeof(entry[propertyName]) === "string" ) ? "'" + entry[propertyName] + "'" : entry[propertyName];
          if (val !== null) vals += propertyName + "=" + val + ",";
        }
        vals = vals.slice(0, vals.length - 1);
        sql = "UPDATE " + args.table + " SET "  + vals + " WHERE " + args.externalIdPath + " = '" + entry[args.externalIdPath] +"'";
        console.log("sql", sql);
        smartstore.get(sql, [], function(err, row){
          if (!err) {
            console.log("row", row);
            if (args.entries.length ==1) {
              ...
              console.log("entry", entry);
              // we are inserting 1 rec, so may want the soupEntryId
              sql = "SELECT * FROM " + args.table + " WHERE " + args.externalIdPath + " = '" + entry[args.externalIdPath] +"'";
              smartstore.get(sql, [], function(err, row){
                if (!err) {
                  console.log("row", row);
                  success([row]);
          ...
          if (i == args.entries.length && args.entries.length != 1) {
            success(rows);
          }
        });
      } else {
  ...
  //     for(var propertyName in args.entries[i]) {
  //       cols += propertyName + ",";
  //       vals += args.entries[i][propertyName] + ",";
  //     }
  //     console.log("cols", cols);
  //     console.log("vals", vals);
  //     console.log("stmt", i, stmt);
  //     // colDefs.slice(0, colDefs.length - 1)
  //     cols = "Name, CurrentValue";
  //     vals = "'Test', 'Test2'";
  //     stmt.run([cols, vals], function(stmt2){
  //       console.log("stmt2", stmt2);
  //       // if (!err) {
  //       //   (row) ? success(true) : success(false);
  //       // } else {
...
function smartstoreQuerySoup(args, success) {
  ...
  smartstore.all(mySql, function(err, rows){
      ...
      if (!err) {
        (rows) ? success(rows) : success(false);
      } else {
        console.log("err", err);
        success(false);
      }
