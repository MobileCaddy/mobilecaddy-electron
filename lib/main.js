 ...
 * @description Show about window - with hidden recovery link
 * @return {[type]} [description]
 */
function showAboutWindow() {
  // Create the browser window.
  aboutWindow = new BrowserWindow({icon: process.resourcesPath + '/app/icon.png', width: 400, height: 400, resizable: false, movable: false,minimizable: false, frame: false, parent: mainWindow})

  // and load the launcher.html of the app
 aboutWindow.loadURL('file://' + launcherHTMLPath)

 aboutWindow.on('blur', (e, cmd) => {
  aboutWindow.close();
  });
}
 ...
 * @description Loads up the remote MCRecovery page
 * @return {[type]} [description]
 */
function showRecovery() {
  ...
  storage.get('alreadyLoggedIn', function(error, data) {
    ...
    const org_id = getOrgIdFromQueryString();
    let instanceUrl = getInstanceFromQueryString();
    var recoveryURL = instanceUrl + '/apex/MCRecovery';
...
mobilecaddy.showAboutWindow = showAboutWindow
