...
const pjson = require(p.join(p.dirname(module.parent.filename),'package.json'));
  ...
  storage.has('alreadyLoggedIn', function(error, hasKey) {
    ...
    if (hasKey) {
      ...
      authWindow.webContents.on('did-finish-load', function(event, newUrl) {
        ...
        if ( authShouldClose ) {
          authWindow.close();
          // mainWindow.loadURL(`file://${__dirname}/launcher2.html?` + queryString + '&buildName=' + pjson.buildName);
