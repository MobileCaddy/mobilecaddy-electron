    ...
    for (var i = 0; i < args.entries.length;  i++) {
      ...
      if (args.entries[i]._soupEntryId) {
        ...
        for(var propertyName in args.entries[i]) {
          vals += propertyName + "='" + args.entries[i][propertyName] + "',";
        }
        vals = vals.slice(0, vals.length - 1);
        sql = "UPDATE " + args.table + " SET "  + vals + " WHERE " + args.externalIdPath + " = " + args.entries[i][args.externalIdPath];
        console.log("sql", sql);
